...
class FastTextRank4Sentence(object):
    ...
    def summarize(self, text, n):
        text = text.replace('\n', '。')
        text = text.replace('\r', '')
        text = util2.as_text(text)  # 处理编码问题
        tokens = util2.cut_sentences(text)
        # sentences用于记录文章最原本的句子，sents用于各种计算操作
        sentences, sents = util2.cut_filter_words(tokens, self.__stop_words, self.__use_stopword)
        self.sens = sentences

        if self.__use_w2v:
            sents = self.filter_dictword(sents)
        graph = self.create_graph_sentence(sents, self.__use_w2v)
        scores = util2.weight_map_rank(graph, self.__max_iter, self.__tol)
        ...
        for i in range(min(n, len(sent_selected))):  # 防止越界
            sent_index.append(sent_selected[i][1])  # 添加句子在原来文章中的下标

        if not sent_index:
            return [None], [None]  # 防止越界
        return [a[0] for a in sent_selected[:len(sent_index)]], [sentences[i] for i in sent_index]
        ...
        for i, j in product(range(num), repeat=2):
            if i != j:
                if use_w2v:
                    ...
                    board[i][j] = util2.two_sentences_similarity(word_sent[i], word_sent[j])
        return board
    ...
    def compute_similarity_by_avg(self, sents_1, sents_2):
        ...
        similarity = util2.cosine_similarity(vec1 / len(sents_1), vec2 / len(sents_2))
        return similarity
