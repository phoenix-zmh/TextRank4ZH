...
def cut_sentences(sentence):
    ...
    for ch in sentence:  # 遍历字符串中的每一个字
        tmp.append(ch)
        if sentence_delimiters.__contains__(ch):  # 避免一个符号作为一个句子
            tmp1 = ''.join(tmp)
...
def weight_map_rank(weight_graph, max_iter, tol):
    ...
    # 初始分数设置为0.5
    # 初始化每个句子的分子和老分数
    scores = [0.5 for _ in range(len(weight_graph))]  # scores = [0.5]*len(weight_graph)
    old_scores = [0.0 for _ in range(len(weight_graph))]
    denominator = calculate_degree(weight_graph)

    # 测试区
    size_len = len(weight_graph)
    size_wide = len(weight_graph[0])
    print('size_len', size_len)
    print('size_wide', size_wide)

    idx = []
    for n in range(size_len):
        for m in range(size_wide):
            if weight_graph[n][m] == 0:
                idx.append('%d | %d' % (n, m))
                # print('n', n, '| m', m)
    print(idx[:5])
...
def calculate_degree(weight_graph):
    ...
    denominator = [0.0]*length
    for j in range(length):
        for k in range(length):
            denominator[j] += weight_graph[j][k]  # 出度加权求和
        if denominator[j] == 0:
            denominator[j] = 1.0
...
def combine2(word_list, window=2):
    """构造在window下的单词组合，用来构造单词之间的边。
    ...
    for x in range(0, length-window+1):
        win_words = word_list[x:x+window]  # 存放每个窗口的词
        for n in win_words:
            for m in win_words:
                if n != m:
                    yield (n, m)
